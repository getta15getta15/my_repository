...
output = output + chr(13) + r"root         1  25      0.4       4608  3648 pts/0    Ss   06:15   0:00 bash" 
output = output + chr(13) + r"root         7  20      0.2       8536  4256 pts/0    R+   06:15   0:00 ps aux"
output = output + chr(13) + r"test         8  15      0.7       8536  4256 pts/0    R+   06:15   0:00 ps aux"
...
for line in lines:
    ...
    arr_cpu.update({parts[10]: arr_cpu.get(parts[1], 0) + float(parts[2])})
    arr_mem.update({parts[10]: arr_mem.get(parts[1], 0) + float(parts[3])})
...
report = report + chr(13) + 'Всего CPU используется %:' + "  " + str("{:10.2f}".format(cpu))

report = report + chr(13) + 'Всего памяти используется %:' + "  " + str("{:10.2f}".format(mem))
...
report = report + chr(13) + 'Больше всего памяти использует:' + "  " + ([key for key in top_cpu.keys()][0])[:20]
report = report + chr(13) + 'Больше всего CPU использует:' + "  " + ([key for key in top_mem.keys()][0])[:20]
